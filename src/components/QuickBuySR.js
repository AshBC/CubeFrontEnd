...
function QuickBuySR(props) {
  ...
  return (
    <ul className="buy-page">
      {props.artworks.filter((artwork) => artwork.image !== '')
        .map(artwork => {
          ...
          <li className="list-item" key={artwork.id}>
            <img className="art-image" src={artwork.image} alt={artwork.description}/>
            ...
            <a href={artwork.url} className="seller"><h3>Super Rare ➝ ⟼ ⤑ ⇀</h3></a>
            </div>
          </li>
